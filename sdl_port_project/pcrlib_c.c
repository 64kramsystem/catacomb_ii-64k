...
void ReadJoystick(int joynum, int *xcount, int *ycount) {
  ...
  *ycount = 0;
  ...
  if (joystick[joynum].isgamecontroller) {
    a1 = SDL_GameControllerGetAxis(joystick[joynum].controller,
                                   SDL_CONTROLLER_AXIS_LEFTX);
    a2 = SDL_GameControllerGetAxis(joystick[joynum].controller,
...
ControlStruct ControlJoystick(int joynum) {
  ...
  if (joystick[joynum].isgamecontroller) {
    action.button1 =
        (SDL_GameControllerGetButton(joystick[joynum].controller,
...
void erasewindow(void) { bar(win_xl, win_yl, win_xh, win_yh, ' '); }
...
void UpdateScreen() {
  static Uint32 EGAPalette[16] = {0x000000, 0x0000AA, 0x00AA00, 0x00AAAA,
                                  0xAA0000, 0xAA00AA, 0xAA5500, 0xAAAAAA,
                                  0x555555, 0x5555FF, 0x55FF55, 0x55FFFF,
                                  0xFF5555, 0xFF55FF, 0xFFFF55, 0xFFFFFF};
  static Uint32 CGAPalette[4] = {0x000000, 0x55FFFF, 0xFF55FF, 0xFFFFFF};
...
static const SDL_Scancode DOSScanCodeMap[128] = {0,
                                                 SDL_SCANCODE_ESCAPE,
                                                 SDL_SCANCODE_1,
                                                 SDL_SCANCODE_2,
                                                 SDL_SCANCODE_3,
...
void _savectrls(void) {
  ...
  if ((handle = open(str, O_WRONLY | O_BINARY | O_CREAT | O_TRUNC,
                     S_IREAD | S_IWRITE)) == -1)
    return;
...
void _setupgame(void) {
  ...
  if ((window = SDL_CreateWindow(
           "The Catacomb", bounds.x, bounds.y, mode.w, mode.h,
           windowed ? 0 : SDL_WINDOW_FULLSCREEN_DESKTOP)) == NULL ||
